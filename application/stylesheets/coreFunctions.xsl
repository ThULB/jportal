...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
				...
				<xsl:choose>
					...
					<xsl:when test="contains(substring-after($url,$parAndVal),'&amp;')">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:value-of
									select="concat(substring-before($url,$parAndVal),substring-after($url,concat($parAndVal,'&amp;')))"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
	...
	<xsl:template name="ShortenText">
		...
		<xsl:choose>
			<xsl:when test="string-length($text) > $length">
				<xsl:value-of
					select="concat(substring($text,1,$length),substring-before(substring($text,($length+1)),' '),'...')"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="$href=''">
				...
				<xsl:variable name="RequestURL.WebURLDel">
					<xsl:value-of
						select="concat('/',substring-after($RequestURL.lastPageDel,$WebApplicationBaseURL.lastPageDel))"/>
				</xsl:variable>
				...
				<xsl:variable name="browserAddress_href">
					<xsl:value-of select="$loaded_navigation_xml//item[(@href=$RequestURL.WebURLDel.langDel) or (@href=$RequestURL.langDel)]/@href"/>
				</xsl:variable>
	...
	<xsl:template name="formatISODate">
		...
		<xsl:value-of xmlns:mcrxml="xalan://org.mycore.common.xml.MCRXMLFunctions"
			select="mcrxml:formatISODate( string( $date ),string( $format ),string( $locale ) )"/>
	</xsl:template>
	...
	<xsl:template name="Tokenizer">
		...
		<xsl:message> called Tokenizer with string=
