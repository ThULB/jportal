...
<!-- ============================================== -->
<!-- $Revision: 1.1 $ $Date: 2007-09-07 12:22:13 $ -->
<!-- ============================================== -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="include/@uri[contains(., 'foobar')]">
    <xsl:attribute name="{local-name()}">
    ...
    <xsl:attribute name="{local-name()}">
      <xsl:choose>
        <xsl:when test="$step='author'">
          <xsl:value-of select="'MCRCheckNewDataServlet'" />
        </xsl:when>
        <xsl:when test="$step='commit'">
          <xsl:value-of select="'MCRCheckCommitDataServlet'" />
        </xsl:when>
