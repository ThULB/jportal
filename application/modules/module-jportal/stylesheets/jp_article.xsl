				...
				<td>
					...
					<span id="leaf-published">
						...
						<xsl:variable name="format">
							<xsl:choose>
								<xsl:when
									test="string-length(normalize-space(xalan:nodeset($cXML)/mycoreobject/service/servdates/servdate[@type='createdate']))=4">
									<xsl:value-of select="i18n:translate('metaData.dateYear')"/>
								</xsl:when>
								<xsl:when
									test="string-length(normalize-space(xalan:nodeset($cXML)/mycoreobject/service/servdates/servdate[@type='createdate']))=7">
									<xsl:value-of select="i18n:translate('metaData.dateYearMonth')"/>
								</xsl:when>
								<xsl:when
									test="string-length(normalize-space(xalan:nodeset($cXML)/mycoreobject/service/servdates/servdate[@type='createdate']))=10">
						...
						<xsl:call-template name="formatISODate">
							<xsl:with-param name="date"
								select="xalan:nodeset($cXML)/mycoreobject/service/servdates/servdate[@type='createdate']/text()"/>
						...
						<div id="detailed-derivate-div">
							...
							<table border="0" cellspacing="0" cellpadding="0" id="detailed-view">
								<xsl:call-template name="printMetaDates">
									...
									<xsl:with-param select="i18n:translate('editormask.labels.pub_reference')"
										name="label"/>
								</xsl:call-template>
							</table>
							<table border="0" cellspacing="0" cellpadding="0" id="detailed-view">
