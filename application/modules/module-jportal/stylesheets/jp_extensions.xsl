                    ...
                    <tr>
                        <td align="left">
                            <xsl:if test="number($numChildren)>5">
                                <form id="pageSize" target="_self" action="{$WebApplicationBaseURL}receive/{/mycoreobject/@ID}{$HttpSession}" method="post">
                                    <p>
                                        <select onChange="document.getElementById('pageSize').submit()" name="XSL.toc.pageSize.SESSION" size="1">
                                            <option value="5">
                ...
                <td class="metavalue">
                    ...
                    <xsl:for-each select="xalan:nodeset($classXML)/mycoreclass/categories/category">
                        ...
                        <xsl:if test="$nodes[@type=$categID] | $nodes[@xlink:title=$categID]">
                            ...
                            <i>
                                <xsl:value-of
                                    select="xalan:nodeset($classXML)/mycoreclass/categories/category[@ID=$categID]/label[@xml:lang=$CurrentLang]/@text"/> : </i>
                            <br/>
                            <ul>
