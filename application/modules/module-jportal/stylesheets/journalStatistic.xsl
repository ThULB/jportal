...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        ...
        <xsl:for-each select="statistic[@date=$journalStatistic.date]">
            <xsl:variable name="chartLabelstemp">
                <xsl:for-each select="journal">
                    <!--                     <xsl:sort select="./numberOfObjects/total/@percent" order="descending" /> -->
                    <xsl:choose>
        ...
        <xsl:variable name="journal-type">
            <xsl:choose>
                <xsl:when test="@type='fully'">
                    <xsl:value-of
                        select="concat(' (vollständige Erschliessung, mit insgesamt ', number(numberOfObjects/total/text())-number(numberOfObjects/missing/text()), ' Artikeln)')" />
                </xsl:when>
    ...
    <xsl:template name="journalStatistic.objectDev">
        ...
        <xsl:variable name="chartURL.values.tmp">
            <xsl:for-each select="/journalStatistic/statistic[number(@date) &lt;= number($journalStatistic.date)]">
                ...
                <xsl:choose>
                    <xsl:when test="journal/@type='fully'">
                        <xsl:value-of
                            select="concat(number(journal[@id=$jID]/numberOfObjects/total/text())-number(journal[@id=$jID]/numberOfObjects/missing/text()),',')" />
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="concat(./journal[@id=$jID]/numberOfObjects/total/text(),',')" />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:for-each>
        </xsl:variable>
        ...
        <xsl:variable name="chartURL.label">
            <xsl:value-of select="substring($chartURL.label.tmp,1,string-length($chartURL.label.tmp)-1)" />
        </xsl:variable>
        ...
        <p style="text-align: center;">
            <img
                src="{concat($chartBaseUrl,'cht=lc&amp;chd=t:',$chartURL.values,'&amp;chs=',$chartSize.objectDev,'&amp;chl=',$chartURL.label,'&amp;chco=0000ff')}" />
        </p>
    ...
    <xsl:template name="journalStatistic.consistancyDev">
        ...
        <p style="text-align: center;">
            <img
                src="{concat($chartBaseUrl,'cht=lc&amp;chd=t:',$chartURL.values,'&amp;chs=',$chartSize.consistancyDev,'&amp;chl=',$chartURL.label,'&amp;chco=',$chartURL.colors,'&amp;chm=',$fillColors)}" />
        </p>
