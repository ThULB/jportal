...
public class MCRDublicateFinderServlet extends MCRServlet {
    ...
    private static final String RED_MAP = MCRConfiguration.instance().getString("MCR.basedir") + FS + "build" + FS + "webapps" + FS + "redundancy.xml";
    ...
    public synchronized void doGetPost(MCRServletJob job) throws JDOMException, IOException {
        ...
        Document redunMap = builder.build(RED_MAP);
        LOGGER.debug("read dublicate list from file=" + RED_MAP);
        ...
        // if a doublet shall be closed, verify if not already done
        ...
        if (job.getRequest().getParameter("status") != null && !job.getRequest().getParameter("status").equals("open")
            ...
            changeable = false;
        LOGGER.debug("changeable=" + changeable + ", id=" + id);

        // edit doublet entry
        if (MCRAccessManager.checkPermission(id, "writedb") && changeable) {
            ...
            FileOutputStream fos = new FileOutputStream(new File(RED_MAP));
            XMLOutputter xo = new XMLOutputter(format);
            xo.output(redunMap, fos);
            fos.flush();
            fos.close();
            LOGGER.debug("saved changed  dublicate list to file=" + RED_MAP);
        } else
            LOGGER.info("NOT changed mode of doublet=" + id + ", because already changed or no permission");

        // send to client
        getLayoutService().doLayout(job.getRequest(), job.getResponse(), redunMap);
