...
public class MCRJPortalURIGetJournalID implements MCRURIResolver.MCRResolver {
    ...
    /**
     * Syntax:
     * <code>jportal_getJournalID:XPath2BeFilled
     * 
    ...
    public Element resolveElement(String uri) {
        ...
        returnXML.addContent(new Element("hidden")
        		.setAttribute("var", xPath2BeFilled)
        		.setAttribute("default", getID()));
	...
	/**
	 * @param webSiteContext: Address of static xml file in navigation.xml that has a template assignment which fits a 
	...
	private static String getWebSiteContext(String lastPage) {
		...
		if (webSiteContextElem.getTextTrim().equals("root")) {
			...
			XMLOutputter out = new XMLOutputter();
			try {
				LOGGER.debug("Found websitecontext=");out.output(webSiteContextElem,System.out);
			} catch (IOException e) {
				e.printStackTrace();
			}	
		}
