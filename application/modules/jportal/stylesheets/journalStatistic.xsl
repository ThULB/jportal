...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="journalStatistic.view.objectListing"
		select="'false'" />
	<xsl:param name="journalStatistic.view.objectListing.journalID" />
	<xsl:param name="journalStatistic.showJournalwithoutActivity"
		select="'true'" />
	<xsl:param name="journalStatistic.view.objectListing.numberOfLabels"
		select="20" />
	<xsl:param name="journalStatistic.view.objectListing.numberOfValues"
		select="50" />
	<xsl:param name="journalStatistic.percentageOfVisibilty"
		select="'1'" />
	<xsl:param name="journalStatistic.dateDerivateCount"
		select="'1213113971040'" />
	...
	<xsl:variable name="chartBaseUrl"
		select="'http://chart.apis.google.com/chart?'" />
	<xsl:variable name="journalStatistic.sourceUrl"
		select="concat($WebApplicationBaseURL,'journalStatistic.xml')" />
	...
	<xsl:variable name="numberOfLabels">
		<xsl:value-of
			select="$journalStatistic.view.objectListing.numberOfLabels" />
	</xsl:variable>
	<!-- 
	...
	<xsl:variable name="numberOfValues">
		<xsl:value-of
			select="$journalStatistic.view.objectListing.numberOfValues" />
	</xsl:variable>
	...
	<xsl:template name="journalStatistic.main">
		...
		<xsl:choose>
			<xsl:when
				test="$journalStatistic.view.objectListing='true'">
				<xsl:call-template
					name="journalStatistic.objectListing" />
			</xsl:when>
			<xsl:otherwise>
				<a name="toc" />
				<xsl:call-template
					name="journalStatistic.selectDateSpace" />
				<xsl:call-template
					name="journalStatistic.selectActivitySwitch" />
				<xsl:call-template name="journalStatistic.toc" />
				<xsl:choose>
					<xsl:when
						test="$showStats='gesamt' or $showStats='proz' or $showStats='gesamtAnz' or $showStats='ai' or $showStats='derivates'">
						<p>
							<a name="gesamt" />
							<b>Gesamtübersicht:</b>
							<a
								style="margin-left: 20px; border:1px solid black;"
								href="#toc">
								^^ zurück ^^
							</a>
						</p>
						<div
							style="border: 2px solid black; width: 900px;">
							<xsl:call-template
								name="journalStatistic.total" />
							<xsl:if
								test="$showStats='gesamt' or $showStats='ai'">
								<xsl:call-template
									name="journalStatistic.PieChart.ArticleOverTime" />
							</xsl:if>
							<xsl:if
								test="$showStats='gesamt' or $showStats='derivates'">
								<xsl:call-template
									name="journalStatistic.get.DerivateOverallStats" />
							</xsl:if>
						</div>
					</xsl:when>
					<xsl:when
						test="$showStats='detail' or contains($showStats,'jportal')">
						<p>
							<a name="detail" />
							<b>Detailansicht:</b>
							<a
						...
						<xsl:call-template
							name="journalStatistic.statistics" />
					</xsl:when>
	...
	<xsl:template name="journalStatistic.toc">
		...
		<p>
			<a
				href="?XSL.selecter=gesamt&amp;XSL.journalStatistic.dateReceived.From={$journalStatistic.date.From}&amp;XSL.journalStatistic.dateReceived.Till={$journalStatistic.date.Till}#gesamt">
				<b>Gesamtübersicht:</b>
			</a>
			<ul>
				<li>
				...
				<li>
					<a
						...
						Anzahl der Digitalisate auf dem
						Zeitschriftenserver
					</a>
				</li>
			</ul>
		...
		<p>
			<a
				href="?XSL.selecter=detail&amp;XSL.journalStatistic.dateReceived.From={$journalStatistic.date.From}&amp;XSL.journalStatistic.dateReceived.Till={$journalStatistic.date.Till}#detail">
				<b>Detailansicht:</b>
			</a>
			<ul>
				<xsl:for-each
					select="statistic[number(@date) = number($journalStatistic.date.Till)]/journal">
					...
					<xsl:variable name="activityOfJournal">
						<xsl:choose>
							<xsl:when
								test="$journalStatistic.showJournalwithoutActivity='false'">
								<xsl:value-of
									xmlns:mcrxml="xalan://org.mycore.frontend.cli.MCRJournalStatsUtilities"
									select="mcrxml:returnAI( string($jID), string($journalStatistic.date.From), string($journalStatistic.date.Till) )" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="''" />
							</xsl:otherwise>
					...
					<xsl:choose>
						<xsl:when
							test="($journalStatistic.showJournalwithoutActivity='false') and ($activityOfJournal &lt;= 0)">
						</xsl:when>
						<xsl:otherwise>
							<li>
								<a
									href="?XSL.selecter={$jID}&amp;XSL.journalStatistic.dateReceived.From={$journalStatistic.date.From}&amp;XSL.journalStatistic.dateReceived.Till={$journalStatistic.date.Till}#{$jID}">
		...
		<p>
			...
			<ul>
				<xsl:for-each
					...
					<li>
						<a
							href="{$WebApplicationBaseURL}receive/{@id}">
							Artikel
							<xsl:value-of select="@id" />
						</a>
					</li>
	...
	<xsl:template name="journalStatistic.statistics">
		...
		<br />
		<xsl:apply-templates
			select="statistic[@date=$journalStatistic.date.Till]">
	...
	<xsl:template name="journalStatistic.selectDateSpace">
		<p>
			<form method="get" action="{$journalStatistic.sourceUrl}"
				target="_self" id="sortByDate1">
				<table>
					<tr>
						<td colspan="2">
					...
					<tr>
						<td>
							von:
							<select size="1"
								name="XSL.journalStatistic.dateReceived.From">
								<xsl:for-each select="statistic">
									<xsl:sort select="@date"
										order="descending" />
									<option value="{@date}">
										<xsl:if
											test="@date=$journalStatistic.date.From">
											<xsl:attribute
						...
						<td>
							bis:
							<select size="1"
								name="XSL.journalStatistic.dateReceived.Till">
								<xsl:for-each select="statistic">
									<xsl:sort select="@date"
										order="descending" />
									<option value="{@date}">
										<xsl:if
											test="@date=$journalStatistic.date.Till">
											<xsl:attribute
												name="selected"><xsl:value-of
													select="'selected'" />
                                            </xsl:attribute>
						...
						<td style="width: 50px;">
							<input type="hidden" name="XSL.selecter"
								value="{$showStats}" />
						</td>
						<td>
	...
	<xsl:template name="journalStatistic.selectActivitySwitch">
		<p>
			<form method="post"
				action="{$journalStatistic.sourceUrl}?XSL.selecter={$showStats}&amp;XSL.journalStatistic.dateReceived.From={$journalStatistic.date.From}&amp;XSL.journalStatistic.dateReceived.Till={$journalStatistic.date.Till}#{$showStats}"
				target="_self" id="activtySwitch">
				<table>
					<tr>
						<td colspan="2">
							<strong>
								Zeitung ohne Aktivität im ausgewählten
								Zeitraum anzeigen:
							</strong>
						</td>
					</tr>
					<tr>
						<td>
							<input type="radio"
								name="XSL.journalStatistic.showJournalwithoutActivity.SESSION"
								value="true"
								onChange="document.getElementById('activtySwitch').submit()">
								<xsl:if
									test="$journalStatistic.showJournalwithoutActivity='true'">
	...
	<xsl:template name="journalStatistic.total">
		<xsl:if test="$showStats='gesamt' or $showStats='proz'">
			<xsl:for-each
				select="statistic[number(@date) = number($journalStatistic.date.Till)]">
				<br />
				<a name="proz" />
				<b style="padding: 5px;">
				...
				<xsl:variable name="chartLabelstemp">
					<xsl:for-each select="journal">
						<xsl:sort
							select="./numberOfObjects/total/@percent" data-type="number"
							order="descending" />
						<xsl:if
							test="numberOfObjects/total/@percent &gt;= $journalStatistic.percentageOfVisibilty">
							<xsl:choose>
								<xsl:when
									test="string-length(@name)>20">
									<xsl:value-of
				...
				<xsl:variable name="chartLabelsAll">
					<xsl:value-of
						select="concat('&amp;chl=',substring($chartLabelstemp,2))" />
				</xsl:variable>
				<xsl:variable name="chartValuestemp">
					<xsl:for-each select="journal">
						...
						<xsl:if
							test="numberOfObjects/total/@percent &gt;= $journalStatistic.percentageOfVisibilty">
							<xsl:value-of
								select="concat(',', numberOfObjects/total/@percent)" />
						</xsl:if>
					</xsl:for-each>
				</xsl:variable>
				<xsl:variable name="chartValuesAll">
					<xsl:value-of
						select="concat('&amp;chd=t:',substring($chartValuestemp,2))" />
				</xsl:variable>
				...
				<xsl:variable name="chartParamsAll"
					select="'chs=810x300&amp;cht=p'" />
				<xsl:variable name="CompletePieChartURLAll"
					select="concat($chartBaseUrl, $chartParamsAll, $chartValuesAll, $chartLabelsAll )" />
		...
		<xsl:if test="$showStats='gesamt' or $showStats='gesamtAnz'">
			<xsl:variable name="allSums">
				<xsl:for-each
					...
					<node>
						<xsl:value-of
							select="sum(journal[@type='fully']/numberOfObjects/total/text())" />
					</node>
				</xsl:for-each>
			</xsl:variable>
			...
			<xsl:variable name="theoreticMinimum">
				<xsl:value-of
					select="($minSum - (($maxMinDistance div 75)*25))" />
			</xsl:variable>

			<xsl:variable name="totalSum">
				<xsl:for-each select="xalan:nodeset($allSums)/node">
					<xsl:value-of
						select="concat( (((text() - $theoreticMinimum)*100) div ($maxSum - $theoreticMinimum)) ,',')" />
				</xsl:for-each>
			</xsl:variable>
			<xsl:variable name="totalLC.valueCounter">
				<xsl:value-of
					select="count(xalan:nodeset($allSums)/node)" />
			</xsl:variable>

			<xsl:variable name="totalLC.values">
				<xsl:value-of
					select="substring($totalSum,1,string-length($totalSum)-1)" />
			</xsl:variable>
			...
			<xsl:variable name="label2">
				<xsl:value-of
					select="round($minSum+($maxMinDistance*0.33))" />
			</xsl:variable>
			<xsl:variable name="label3">
				<xsl:value-of
					select="round($minSum+($maxMinDistance*0.66))" />
			</xsl:variable>
			...
			<xsl:variable name="chartURL.ylabel">
				<xsl:value-of
					select="concat('|',0,'|',$label1,'|',$label2,'|',$label3,'|',$label4)" />
			</xsl:variable>
			...
			<b style="padding: 5px;">
				<u>
					<xsl:value-of
						select="concat('Gesamtanzahl der Artikel über die Zeit (Differenz = ',$maxMinDistance,'):')" />
				</u>
			</b>
			<a style="margin-left: 20px; border:1px solid black;"
				href="#toc">
			...
			<xsl:variable name="totalLC.values.decreased">
				<xsl:choose>
					<xsl:when
						test="$totalLC.valueCounter>$numberOfValues">
						<xsl:value-of
							xmlns:mcrxml="xalan://org.mycore.frontend.cli.MCRJournalStatsUtilities"
	...
	<xsl:template match="journal">
		...
		<xsl:variable name="activityOfJournal">
			<xsl:value-of
				xmlns:mcrxml="xalan://org.mycore.frontend.cli.MCRJournalStatsUtilities"
				select="mcrxml:returnAI( string($journal-id), string($journalStatistic.date.From), string($journalStatistic.date.Till) )" />
		</xsl:variable>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="derNumber">
					<xsl:value-of
						select="sum(derivates/derivate/@number)" />
				</xsl:variable>
				<xsl:variable name="derSizeTemp">
					<xsl:value-of
						select="sum(derivates/derivate/@size)" />
				</xsl:variable>

				<xsl:variable name="derSize">
					<xsl:choose>
						<xsl:when
							test="string-length($derSizeTemp) &lt; 4">
							<xsl:value-of
								select="concat($derSizeTemp , ' Byte')" />
						</xsl:when>
						<xsl:when
							test="string-length($derSizeTemp) &lt; 7">
							<xsl:variable name="sizeTemp">
								<xsl:value-of
									select="$derSizeTemp div 1000" />
							</xsl:variable>
							<xsl:value-of
								select="concat(substring-before($sizeTemp,'.'),'.',substring(substring-after($sizeTemp,'.'),1,2), ' KiloByte')" />
						</xsl:when>
						<xsl:when
							test="string-length($derSizeTemp) &lt; 10">
							<xsl:variable name="sizeTemp">
								<xsl:value-of
									select="$derSizeTemp div 1000000" />
							</xsl:variable>
							<xsl:value-of
								select="concat(substring-before($sizeTemp,'.'),'.',substring(substring-after($sizeTemp,'.'),1,2), ' MegaByte')" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:variable name="sizeTemp">
								<xsl:value-of
									select="$derSizeTemp div 1000000000" />
							</xsl:variable>
							<xsl:value-of
								select="concat(substring-before($sizeTemp,'.'),'.',substring(substring-after($sizeTemp,'.'),1,2), ' GigaByte')" />
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
				...
				<div style="width: 900px; border: 2px solid black;">
					...
					<xsl:call-template
						name="journalStatistic.currentlyState">
						<xsl:with-param name="journalType"
							select="$journal-type" />
					</xsl:call-template>
					<br />
					<br />
					...
					<b style="padding: 5px;">
						<xsl:value-of
							select="concat('Aktivitätsindex der Zeitung im ausgewählten Zeitraum: ',$activityOfJournal)" />
					</b>
					<br />
					<br />
					<!-- consistancy development -->
					<xsl:call-template
						name="journalStatistic.consistancyDev">
					...
					<!-- Object development -->
					<xsl:call-template
						name="journalStatistic.objectDev">
						<xsl:with-param name="jID" select="@id"></xsl:with-param>
					</xsl:call-template>
					<br />
					<br />
					...
					<xsl:call-template
						name="journalStatistic.derivateDev">
						<xsl:with-param name="jID" select="@id"></xsl:with-param>
					</xsl:call-template>
				</div>
				<br />
	...
	<xsl:template name="journalStatistic.objectDev">
		...
		<xsl:variable name="firstTotaltemp">
			<xsl:value-of
				select="/journalStatistic/statistic[number(@date) = number($journalStatistic.date.From)]/journal[@id=$jID]/numberOfObjects/total" />
		</xsl:variable>
		...
		<xsl:variable name="lastTotal">
			<xsl:value-of
				select="/journalStatistic/statistic[number(@date) = number($journalStatistic.date.Till)]/journal[@id=$jID]/numberOfObjects/total" />
		</xsl:variable>
		...
		<xsl:variable name="lastFirstDistance">
			<xsl:choose>
				<xsl:when test="$lastFirstDistancetemp &gt; 0">
					<xsl:value-of
						select="concat('+',$lastFirstDistancetemp)" />
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="headline">
			<xsl:choose>
				<xsl:when test="@type='fully'">
					<xsl:value-of
						select="concat('Zahl der Artikel im zeitlichen Verlauf (Differenz = ',$lastFirstDistance ,') :')" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of
						select="concat('Zahl der Bände im zeitlichen Verlauf (Differenz = ',$lastFirstDistance ,') :')" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="theoreticMinimum">
			<xsl:choose>
				<xsl:when test="$maxMinDistance!='0'">
					<xsl:value-of
						select="($minTotal - (($maxMinDistance div 75)*25))" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="0" />
				</xsl:otherwise>
			...
			<xsl:for-each
				...
				<xsl:variable name="tempValues">
					<xsl:choose>
						...
						<xsl:when
							test="(number(journal[@id=$jID]/numberOfObjects/total/text())) and (journal[@id=$jID]/numberOfObjects/total/text() = '0')">
							<xsl:value-of select="0" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when
									...
									<xsl:value-of
										select="(((number(journal[@id=$jID]/numberOfObjects/total/text())-$minTotal) div $maxMinDistance*75)+25)" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="0" />
								</xsl:otherwise>
		...
		<xsl:variable name="label2">
			<xsl:choose>
				<xsl:when test="$maxMinDistance!='0'">
					<xsl:value-of
						select="round($minTotal+($maxMinDistance*0.33))" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''" />
				</xsl:otherwise>
		...
		<xsl:variable name="label3">
			<xsl:choose>
				<xsl:when test="$maxMinDistance!='0'">
					<xsl:value-of
						select="round($minTotal+($maxMinDistance*0.66))" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="''" />
				</xsl:otherwise>
	...
	<xsl:template name="journalStatistic.consistancyDev">
		...
		<xsl:variable name="CompleteDiff">
			<xsl:choose>
				<xsl:when test="$CompleteDiffTemp &gt; 0">
					<xsl:value-of
						select="concat('+',$CompleteDiffTemp)" />
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="IncompleteDiff">
			<xsl:choose>
				<xsl:when test="$IncompleteDiffTemp &gt; 0">
					<xsl:value-of
						select="concat('+',$IncompleteDiffTemp)" />
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="headline">
			<xsl:choose>
				<xsl:when
					test="/journalStatistic/statistic[(number(@date) = number($journalStatistic.date.Till))]/journal[@id=$jID]/@type='fully'">
					<xsl:value-of
		...
		<xsl:variable name="chartURL.values">
			<xsl:call-template
				name="journalStatistic.consistancyDev.collectValues">
	...
	<xsl:template name="journalStatistic.derivateDev">
		...
		<xsl:variable name="headline">
			<xsl:value-of
				select="'Übersicht über die Derivate der Zeitschrift:'" />
		</xsl:variable>

		<xsl:if
			test="count(/journalStatistic/statistic[(number(@date) = number($journalStatistic.date.Till))]/journal[@id=$jID]/derivates/derivate) &gt; 0">
			...
			<xsl:for-each
				select="/journalStatistic/statistic[number(@date) = number($journalStatistic.date.Till)]/journal[@id=$jID]/derivates/derivate">
				<xsl:call-template
					name="journalStatistic.singleDerivateDev">
					<xsl:with-param name="jID" select="$jID" />
					<xsl:with-param name="type" select="@type" />
					<xsl:with-param name="actualSizes"
						select="$actualSizes" />
				</xsl:call-template>
			</xsl:for-each>
		</xsl:if>
	...
	<xsl:template name="journalStatistic.singleDerivateDev">
		...
		<xsl:variable name="derSizeTemp">
			<xsl:value-of
				select="xalan:nodeset($actualSizes)/values/size[@type=$type]/text()" />
		</xsl:variable>
		...
		<xsl:variable name="derSize">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:variable name="sizeTemp">
						<xsl:value-of
							select="$derSizeTemp div 1000000000000" />
					</xsl:variable>
					<xsl:value-of
						select="concat(substring-before($sizeTemp,'.'),'.',substring(substring-after($sizeTemp,'.'),1,2), 'TeraByte')" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="headline">
			<xsl:value-of
				select="concat('Typ: ', $type, '; Größe: ',$derSize,'; Differenz=',$lastFirstDistance,';')" />
		</xsl:variable>
	...
	<xsl:template name="chartURL.get.ComplDiff">
		...
		<xsl:variable name="allComplete">
			<xsl:for-each
				...
				<complete>
					<xsl:value-of
						select="number(journal[@id=$jID]/numberOfObjects/complete/text())" />
				</complete>
			</xsl:for-each>
		...
		<xsl:variable name="maxTotal">
			<xsl:value-of
				select="xalan:nodeset($allComplete)/complete[position()=last()]" />
		</xsl:variable>
		<xsl:variable name="minTotal">
			<xsl:choose>
				<xsl:when
					test="number(xalan:nodeset($allComplete)/complete[position()=1])">
					<xsl:value-of
						select="number(xalan:nodeset($allComplete)/complete[position()=1])" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="0" />
				</xsl:otherwise>
	...
	<xsl:template name="chartURL.get.IncomplDiff">
		...
		<xsl:variable name="allIncomplete">
			<xsl:for-each
				...
				<incomplete>
					<xsl:value-of
						select="journal[@id=$jID]/numberOfObjects/incomplete/text()" />
				</incomplete>
			</xsl:for-each>
		</xsl:variable>
		<xsl:variable name="maxTotal">
			<xsl:value-of
				select="xalan:nodeset($allIncomplete)/incomplete[position()=last()]" />
		</xsl:variable>
		<xsl:variable name="minTotal">
			<xsl:choose>
				<xsl:when
					test="number(xalan:nodeset($allIncomplete)/incomplete[position()=1])">
					<xsl:value-of
						select="number(xalan:nodeset($allIncomplete)/incomplete[position()=1])" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="0" />
				</xsl:otherwise>
	...
	<xsl:template name="chartURL.get.MissingDiff">
		...
		<xsl:variable name="allMissing">
			<xsl:for-each
				...
				<missing>
					<xsl:value-of
						select="journal[@id=$jID]/numberOfObjects/missing/text()" />
				</missing>
			</xsl:for-each>
		</xsl:variable>
		<xsl:variable name="maxTotal">
			<xsl:value-of
				select="xalan:nodeset($allMissing)/missing[position()=last()]" />
		</xsl:variable>
		<xsl:variable name="minTotal">
			<xsl:choose>
				<xsl:when
					test="number(xalan:nodeset($allMissing)/missing[position()=1])">
					<xsl:value-of
						select="number(xalan:nodeset($allMissing)/missing[position()=1])" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="0" />
				</xsl:otherwise>
	...
	<xsl:template name="chartURL.fillColors">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat('b,',$color.complete,',0,1,0|b,',$color.incomplete,',1,2,0')" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="chartURL.colors">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat($color.complete,',',$color.incomplete,',',$color.zero)" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="chartURL.label">
		...
		<xsl:value-of
			select="substring($chartURL.label.tmp,1,string-length($chartURL.label.tmp))" />
	</xsl:template>
	...
	<xsl:template
		...
		<xsl:variable name="values.zero">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of
						select="substring($values.zero.tmp,1,string-length($values.zero.tmp)-1)" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- missings -->
		<xsl:variable name="values.missing.tmp">
			<xsl:for-each
				...
				<xsl:variable name="missing.value">
					<xsl:choose>
						<xsl:when
							test="number(./journal[@id=$journalID]/numberOfObjects/missing/@percent)">
							<xsl:value-of
								select="number(./journal[@id=$journalID]/numberOfObjects/missing/@percent)" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="0" />
						</xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="contains($missing.value,'.')">
						<xsl:value-of
							select="concat(substring-before($missing.value,'.'),',')" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of
							select="concat($missing.value,',')" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:for-each>
		</xsl:variable>
		<xsl:variable name="values.missing">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of
						select="substring($values.missing.tmp,1,string-length($values.missing.tmp)-1)" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- incompletes -->
		<xsl:variable name="values.incomplete.tmp">
			<xsl:for-each
				...
				<xsl:variable name="inc.value">
					<xsl:choose>
						<xsl:when
							test="number(./journal[@id=$journalID]/numberOfObjects/incomplete/@percent)">
							<xsl:value-of
								select="number(./journal[@id=$journalID]/numberOfObjects/incomplete/@percent)" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="0" />
						</xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="contains($inc.value,'.')">
						<xsl:value-of
							select="concat(substring-before($inc.value,'.'),',')" />
					</xsl:when>
					<xsl:otherwise>
		...
		<xsl:variable name="values.incomplete">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of
						select="substring($values.incomplete.tmp,1,string-length($values.incomplete.tmp)-1)" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- corrupted total -->
			...
			<xsl:for-each
				...
				<xsl:choose>
					<xsl:when test="contains($cor.value,'.')">
						<xsl:value-of
							select="concat(substring-before($cor.value,'.'),',')" />
					</xsl:when>
					<xsl:otherwise>
		...
		<xsl:variable name="values.complete.ZeroCheck">
			<corrupt>
				<xsl:for-each
					...
					<xsl:variable name="totalValue">
						<xsl:value-of
							select="number(./journal[@id=$journalID]/numberOfObjects/complete/@percent)" />
					</xsl:variable>
					<value>
		...
		<xsl:variable name="values.corruptedTotal">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of
						select="substring($values.corruptedTotal.tmp,1,string-length($values.corruptedTotal.tmp)-1)" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- fully graph -->
		<xsl:variable name="values.100.tmp">
			<xsl:for-each
				...
				<xsl:choose>
					<xsl:when
						test="number(./journal[@id=$journalID]/numberOfObjects/total/text())">
						<xsl:choose>
							<xsl:when
		...
		<xsl:variable name="values.100">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of
						select="substring($values.100.tmp,1,string-length($values.100.tmp)-1)" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<!-- return value -->
		<xsl:choose>
			<xsl:when test="@type='fully'">
				<xsl:value-of
					select="concat($values.100,'|',$values.corruptedTotal,'|',$values.incomplete,'|',$values.missing,'|',$values.zero)" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of
					select="concat($values.100,'|',$values.incomplete,'|',$values.zero)" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="journalStatistic.currentlyState">
			...
			<tr>
				<td width="900" style="font-weight: bold;">
					...
					<a href="{$WebApplicationBaseURL}receive/{@id}">
						<xsl:value-of
							select="concat(@name, $journalType)" />
					</a>
					<a
						style="margin-left: 20px; border:1px solid black;" href="#toc">
						^^ zurück ^^
					</a>
				</td>
			</tr>
		...
		<xsl:variable name="pervalue1"
			select="numberOfObjects/complete/@percent" />
		<xsl:variable name="pervalue2"
			select="numberOfObjects/incomplete/@percent" />
		<xsl:variable name="pervalue3"
			select="numberOfObjects/missing/@percent" />
		<xsl:variable name="absvalue1"
			select="numberOfObjects/complete/text()" />
		<xsl:variable name="absvalue2"
			select="numberOfObjects/incomplete/text()" />
		<xsl:variable name="absvalue3"
			select="numberOfObjects/missing/text()" />
		...
		<xsl:variable name="chartParams"
			select="concat('chs=', '600x', $scale,'&amp;cht=p')" />
		<xsl:variable name="chartValues"
			select="concat('&amp;chd=t:', $pervalue1, ',', $pervalue2, ',', $pervalue3)" />
		<xsl:variable name="chartColor"
			select="concat('&amp;chco=', '44ca20', ',', 'ca2020', ',', 'f96820')" />
		<xsl:variable name="CompletePieChartURL"
			select="concat($chartBaseUrl, $chartParams, $chartValues, $chartColor)" />
		<p style="text-align: center;">
			<img src="{$CompletePieChartURL}" />
			<!-- Legende -->
			<div
				style="border: 1px inset black; width: 220px; margin: 5px;">
				<table>
					<tr>
						<td
							style="border: 1px inset black; background-color: #{$color.complete}; width: 20px;">
							<xsl:value-of select="' '" />
						</td>
						<td>
						...
						<td>
							<xsl:value-of
								select="concat($absvalue1,' - ',$pervalue1, '%')" />
						</td>
					</tr>
					<tr>
						<td
							style="border: 1px inset black; background-color: #{$color.incomplete}">
							<xsl:value-of select="' '" />
						</td>
						<td>
							<xsl:choose>
						...
						<td>
							<xsl:value-of
								select="concat($absvalue2,' - ',$pervalue2, '%')" />
						</td>
					</tr>
					<xsl:if test="@type='fully'">
						<tr>
							<td
								style="border: 1px inset black; background-color: #{$color.missing}">
								<xsl:value-of select="' '" />
							</td>
							<td>
								<xsl:choose>
							...
							<td>
								<xsl:value-of
									select="concat($absvalue3,' - ',$pervalue3, '%')" />
							</td>
						</tr>
					</xsl:if>
				</table>
	...
	<xsl:template name="get.journalStatistic.date.From">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="$journalStatistic.dateReceived.From" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="get.journalStatistic.date.Till">
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="$journalStatistic.dateReceived.Till" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="journalStatistic.PieChart.ArticleOverTime">
		...
		<xsl:variable name="activitySumTemp">
			<ais>
				<xsl:for-each
					select="/journalStatistic/statistic[(number(@date) = number($journalStatistic.date.Till))]/journal">
					<ai>
						<xsl:variable name="jID">
							<xsl:value-of select="@id" />
						</xsl:variable>
		...
		<xsl:variable name="activitySum">
			<xsl:value-of
				select="sum(xalan:nodeset($activitySumTemp)/ais/ai/text())" />
		</xsl:variable>
		<xsl:variable name="chartLabelsTemp">
			<xsl:for-each
				select="xalan:nodeset($activitySumTemp)/ais/ai">
				<xsl:sort data-type="number" order="descending" />
				<xsl:if
		...
		<xsl:variable name="chartValuesTemp">
			<xsl:for-each
				...
				<xsl:if
					test="((text()*100) div $activitySum) &gt;= $journalStatistic.percentageOfVisibilty">
					<xsl:value-of
						select="concat(',',(text()*100) div $activitySum)" />
				</xsl:if>
			</xsl:for-each>
		</xsl:variable>
		<xsl:variable name="chartLabels"
			select="concat('&amp;chl=',substring($chartLabelsTemp,2))" />
		<xsl:variable name="chartParams"
			select="concat('chs=', '800x300', '&amp;cht=p')" />
		<xsl:variable name="chartValues"
			select="concat('&amp;chd=t:', substring($chartValuesTemp,2))" />
		<xsl:variable name="chartColor"
			select="concat('&amp;chco=', '44ca20', ',', 'ca2020', ',', 'f96820')" />
		<xsl:variable name="CompletePieChartURL"
			select="concat($chartBaseUrl, $chartParams, $chartValues, $chartLabels)" />
		<p style="text-align: center;">
			<xsl:choose>
		...
		<div
			style="margin: 5px; padding: 5px; border: 1px solid black; width: 420px;">
			Legende:
			<br />
			Zeitschriftname(Aktivitäten absolut - prozentualer Anteil an
			allen Aktivitäten)
			<br />
			<br />
			Als Aktivität zählt das Neuanlegen eines Objektes, genauso
			wie deren Zustandsänderung. Zustandsänderung bedeutet z.B.
			einem Objekt ein Digitalisat hinzufügen. Da sich hierbei
			aber 2 Werte verändern, wird diese Teilsumme halbiert.
		</div>
	...
	<xsl:template name="journalStatistic.get.DerivateOverallStats">
					...
					<date>
						...
						<xsl:for-each select="journal">
							<journal>
								...
								<test>
									<xsl:value-of
										select="number(derivates/derivate[position() = 1]/@size)" />
								</test>
							</journal>
						</xsl:for-each>
					</date>
		...
		<xsl:variable name="derSizeTemp">
			<xsl:value-of
				select="sum(xalan:nodeset($derivates)/derivates/date[position()=last()]/journal/size/text())" />
		</xsl:variable>
		...
		<xsl:variable name="overallDerNumber">
			<xsl:value-of
				select="sum(xalan:nodeset($derivates)/derivates/date[position()=last()]/journal/number/text())" />
		</xsl:variable>
		...
		<xsl:variable name="derNumberPerDate">
			<xsl:for-each
				select="xalan:nodeset($derivates)/derivates/date">
				<date>
					<xsl:attribute name="number">
                        <xsl:choose>
                            <xsl:when
								test="number(journal/test/text())">
		...
		<xsl:variable name="maxTotal">
			<xsl:for-each
				select="xalan:nodeset($derNumberPerDate)/date/text()">
				<xsl:sort data-type="number" order="descending" />
				<xsl:if test="position()=1">
					<xsl:value-of select="." />
				</xsl:if>
		...
		<xsl:variable name="minTotal">
			<xsl:for-each
				select="xalan:nodeset($derNumberPerDate)/date[@number='true']/text()">
				<xsl:sort data-type="number" order="ascending" />
				<xsl:if test="position()=1">
					<xsl:value-of select="." />
				</xsl:if>
		...
		<xsl:variable name="firstTotaltemp">
			<xsl:value-of
				select="xalan:nodeset($derNumberPerDate)/date[position()=1]/text()" />
		</xsl:variable>
		<xsl:variable name="firstTotal">
			<xsl:choose>
				<xsl:when test="number($firstTotaltemp)">
		...
		<xsl:variable name="lastTotal">
			<xsl:value-of
				select="xalan:nodeset($derNumberPerDate)/date[position()=last()]/text()" />
		</xsl:variable>
