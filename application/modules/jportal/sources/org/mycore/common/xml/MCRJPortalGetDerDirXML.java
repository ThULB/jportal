...
public class MCRJPortalGetDerDirXML implements MCRResolver {
	...
	private static final Logger LOGGER = Logger
			.getLogger(MCRJPortalGetDerDirXML.class);

	/**
	 * Returns a XML directory listing of a derivate and caches the last call.
	 * So, this should speed up results of fulltext search that are within a file. 
	 * Syntax: <code>jportal_getDerDirXML:DerivateID
	 * 
	 * @param uri
	...
	public Element resolveElement(String uri) throws IOException {
		...
		if (!wellURI(uri))
			throw new IllegalArgumentException(
					"Invalid format of uri given to resolve " + URI + "=" + uri);
	...
	private boolean wellURI(String uri) {
		...
		if (parameters.length == 2 && parameters[0].equals(URI)
				&& !parameters[1].equals(""))
			return true;
		return false;
	}
