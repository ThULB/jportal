...
/**
 * Creates a redundancy file from the previous generated checkForDuplicates-file.
...
public class MCRFindDuplicates {
    ...
    public static void findDuplicates() throws Exception {
        Document d = new SAXBuilder().build(new File("checkForDuplicates.xml"));
        if (d == null) {
            LOGGER.error("Couldnt find checkForDuplicates.xml.");
            return;
        }
        ...
        if (type == null) {
            LOGGER.error("Attribute 'type' is not defined.");
            return;
        }
        ...
        FileOutputStream output = new FileOutputStream(ROOT_DIR + "redundancy-" + type + ".xml");
