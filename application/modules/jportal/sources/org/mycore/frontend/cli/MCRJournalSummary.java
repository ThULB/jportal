...
public class MCRJournalSummary extends MCRAbstractCommands {
    ...
    // object
    private static void parseDerivate(MCRFilesystemNode root, MCRObjectID objParentIDtemp, int i) {
        String type = "";
        Long size = 0l;
        ...
        for (int k = 0; k <= maxJouID; k++) {
            ...
            if (journals.containsKey(JournalID)) {
                ...
                Element XMLjournal = new Element("journal").setAttribute("name", journal.getJournalName()).setAttribute("type",
                        journal.getObjectFocus()).setAttribute("id", journal.getJournalID().toString());
                ...
                Total.setAttribute("percent", Double.toString(round((((double) (journal.getAllCounter() - journal
                        .getMissingChildrenCounter()) / (double) MCRJournalStats.getAllObjectsCounter()) * 100), 2, RoundingMode.HALF_EVEN,
                        FormatType.fix)));
                ...
                String complete = journal.getGoodCounter() + " / "
                        + ((float) journal.getGoodCounter() / (float) journal.getAllCounter() * 100) + "%";
                logger.info("  Complete                    : " + complete);
                Element Complete = new Element("complete");
                Complete.setAttribute("percent", Double.toString(round((((double) journal.getGoodCounter() / (double) journal
                        .getAllCounter()) * 100), 2, RoundingMode.HALF_EVEN, FormatType.fix)));
                Complete.setText(Integer.toString(journal.getGoodCounter()));

                String incomplete = journal.getBadCounter() + " / "
                        + ((float) journal.getBadCounter() / (float) journal.getAllCounter() * 100) + "%";
                logger.info("  Incomplete                  : " + incomplete);
                Element Incomplete = new Element("incomplete").setAttribute(
                        "percent",
                        Double.toString(round((((double) journal.getBadCounter() / (double) journal.getAllCounter()) * 100), 2,
                                RoundingMode.HALF_EVEN, FormatType.fix))).setText(Integer.toString(journal.getBadCounter()));
                ...
                for (int a = 0; a < journal.getDerivateTypes().size(); a++) {
                    ...
                    derivate.setAttribute("type", actualType).setAttribute("number", Long.toString(values[0] + 1)).setAttribute("size",
                            size);
    ...
    private static void saveXML(Element newStats, String dir, String targetFile, boolean log) throws IOException, FileNotFoundException,
            JDOMException {
    ...
    public static void createCSV() throws IOException {
        ...
        while (StatisticIt.hasNext()) {
            ...
            while (JournalIt.hasNext()) {
                String JournalID = JournalIt.next().getAttribute("id").getValue();
        ...
        while (StatisticIt2.hasNext()) {
            ...
            while (JournalIt.hasNext()) {
                ...
                value = Integer.valueOf(journal.getChild("numberOfObjects").getChild("incomplete").getValue()).intValue();
                value = value + Integer.valueOf(journal.getChild("numberOfObjects").getChild("missing").getValue()).intValue();
                ...
                if (hasDerivate) {
                    ...
                    while (derivateIt.hasNext()) {
                        value = value + Integer.valueOf(derivateIt.next().getAttribute("number").getValue()).intValue();
                    }
                }
                if (!first) {
                    int valueBefore = ValueMap.get(JournalID);
                    System.out.println("value: " + value + "valueBefore: " + valueBefore);
                    int activity = valueBefore - value;
                    if (activity < 0) {
                        activity = activity * (-1);
                    }
                    StringOfValues.put(JournalID, StringOfValues.get(JournalID) + "#" + Integer.toString(activity));
                }
            ...
            if (!first) {
                for (int k = 0; k < JournalCount; k++) {
                    ...
                    if (!JIDChecklist.containsKey(JournalID)) {
                        StringOfValues.put(JournalID, StringOfValues.get(JournalID) + "#" + "0");
                    }
                }
        ...
        // new CSVWriter
        CSVWriter writer = new CSVWriter(new FileWriter(saveFolder + "/" + CSVFileName), '#', ' ');
