...
jp.solr.FacetDateQuery = function(options) {
  ...
  /*string*/         this.solrSortField = options.solrSortField;
  ...
  if (this.solrSortField == null) {
    this.solrSortField = "published_sort";
  }
};
...
jp.solr.FacetDateQuery.prototype.fetchEarliest = function () {
  return this.fetchDate(this.solrSortField + " asc");
};
...
jp.solr.FacetDateQuery.prototype.fetchLatest = function () {
  return this.fetchDate(this.solrSortField + " desc");
};
...
jp.solr.FacetDateQuery.prototype.fetchDate = function (sort) {
  let url = this.solrURL + "?q=" + this.solrField + ":*&sort=" + sort + "&rows=1&fl=date.published&wt=json";
  for (let fq of this.fqArray) {
    url += "&fq=" + fq;
  }
  return $.ajax({
    url: url,
    ...
    try {
      const date = data.response.docs[0]["date.published"];
...
jp.solr.FacetDateQuery.prototype.getFacetRangeURL = function () {
  const utcFormat = this.utcFormat();
  let solrURL = this.solrURL;
