...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  <xsl:template name="iview.getSupport.hack">
    ...
    <xsl:variable name="firstSupportedFile">
      <xsl:call-template name="iview2.getSupport">
        <xsl:with-param select="$derivID" name="derivID"/>
      </xsl:call-template>
     </xsl:variable>
        ...
        <xsl:choose>
          <xsl:when test="contains($MCR.Module-iview.SupportedContentTypes,$fileType)">
            <xsl:call-template name="iview.getAddress.hack">
              ...
              <xsl:with-param name="firstSupportedFile" select="$firstSupportedFile" />
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="''" />
  ...
  <xsl:template name="iview.getAddress.hack">
    ...
    <xsl:value-of select="concat($WebApplicationBaseURL,'receive/',$objID,'?XSL.view.objectmetadata=false&amp;jumpback=true&amp;maximized=true&amp;page=',$firstSupportedFile)" />
