...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan"
  ...
  <xsl:template match="response" mode="jp.pagination">
    ...
    <xsl:if test="$resultInfo/pages &gt; 1 or ($resultInfo/numFound &lt; 10 and $resultInfo/start != 0)">
      ...
      <xsl:variable name="pageStart" select="math:min(math:max($resultInfo/page, 1), math:max($resultInfo/pages - 2, 1))" />
      <xsl:variable name="pageEnd" select="math:max(math:min($resultInfo/page + 2, $resultInfo/pages), math:min($resultInfo/pages, 3))" />
