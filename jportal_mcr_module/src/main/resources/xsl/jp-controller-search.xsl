...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:encoder="xalan://java.net.URLEncoder"
  ...
  xmlns:mcrxml="xalan://org.mycore.common.xml.MCRXMLFunctions" exclude-result-prefixes="mcrxml jpxml">
  ...
  <xsl:template match="jpsearch" mode="default">
    ...
    <xsl:variable name="trimQt" select="mcrxml:regexp($qt, '^\s+|\s+$/g', '')" />
    <xsl:variable name="queryXML">
      <query>
        <queryTerm value="({{!join from=returnId to=id}}{$trimQt}) OR {$trimQt}" />
