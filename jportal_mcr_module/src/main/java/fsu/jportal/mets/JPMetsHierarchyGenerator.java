...
public class JPMetsHierarchyGenerator extends MCRMETSHierarchyGenerator {
    ...
    private void enhanceOrderLabels(Mets mets) {
        String recordStatus = mets.getMetsHdr().getRecordStatus();
        Boolean overwrite = Optional.ofNullable(recordStatus)
                .map(status -> status.equals("autogenerated"))
                .orElse(false);

        // put the sizes for each available physical div
        this.sizeMap.forEach((logicalDivId, size) -> {
            try {
                MetsUtil.setOrderlabel(mets, logicalDivId, size, overwrite);
            } catch (Exception exc) {
