...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i18n="xalan://org.mycore.services.i18n.MCRTranslation"
    ...
    <xsl:variable name="PageTitle" select="i18n:translate('indexpage.documents.pagetitle')" />
    ...
    <xsl:template name="index.headline">
        <tr valign="top">
            <td class="metaname">
                <xsl:value-of select="i18n:translate('indexpage.documents.headline.select')" />
                <xsl:value-of select="i18n:translate('indexpage.documents.headline.indextitle')" />
            </td>
        </tr>
    </xsl:template>
    ...
    <xsl:variable name="up.url">
        <xsl:value-of select="concat('indexpage',$HttpSession,'?searchclass=',$IndexID,'&amp;mode=',$mode)" />
        <xsl:if test="string-length($search) &gt; 0">
    ...
    <xsl:template match="value">
        ...
        <xsl:variable name="urlAuthor" select="concat($WebApplicationBaseURL,'receive/',$id, $HttpSession)"></xsl:variable>
        <tr>
            ...
            <td class="td1" valign="top" style="padding-right:5px;">
                <a href="{$urlAuthor}">
                    <xsl:value-of select="i18n:translate('indexpage.link.details')" />
                </a>
            </td>
        </tr>
    </xsl:template>
    ...
    <xsl:template match="range">
        <xsl:variable name="url">
            <xsl:value-of select="concat($WebApplicationBaseURL,'indexpage',$HttpSession,'?searchclass=',$IndexID,'&amp;fromTo=', from/@pos,'-', to/@pos,'&amp;mode=',$mode )" />
            <xsl:if test="string-length($search) &gt; 0">
                <xsl:value-of select="concat('&amp;search=',$search)" />
            </xsl:if>
        </xsl:variable>
    ...
    <xsl:template name="IntroText">
        <xsl:value-of select="i18n:translate('indexpage.documents.introtext')" />
        <p>
            <xsl:value-of select="' | '" />
            <xsl:for-each select="xalan:nodeset($AtoZ)/search">
                <a href="{$WebApplicationBaseURL}indexpage{$HttpSession}?searchclass={$IndexID}&amp;mode={$mode}&amp;search={@prefix}">
