...
<xsl:stylesheet version="1.0"
			...
			<body>
				<table id="topmenu" cellspacing="0" cellpadding="0"
						...
						<td id="head-top">
							<div id="navi_below_cell">
								<xsl:call-template
									...
									<!-- use pixel values -->
									<xsl:with-param name="seperatorChar"
										select="'|'"/>
								</xsl:call-template>
							</div>
						</td>
					...
					<tr id="head-middle">
						<td id="sfb-logo">
							<img
								src="{$WebApplicationBaseURL}templates/master/{$template}/IMAGES/logo_sfb_final.png"
								alt="SFB-Logo">
							</img>
						</td>
					</tr>
				...
				<table id="menuContentWrapper" cellspacing="0" cellpadding="0" border="0">
					<tr>
						...
						<td id="contentArea">
							<xsl:call-template
								name="template_sfb580.write.content" />
						</td>
					</tr>
				</table>
	...
	<xsl:template name="template_sfb580.userInfo">
		...
		<xsl:value-of
			select="concat(i18n:translate('users.user'),': ')" />
		<a href="{$LoginURL}">
