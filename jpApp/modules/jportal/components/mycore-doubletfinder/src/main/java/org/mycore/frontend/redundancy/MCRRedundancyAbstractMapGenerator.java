...
public abstract class MCRRedundancyAbstractMapGenerator {
    ...
    protected Map<String, String> getCompareCriteria(MCRHit mcrHit) {
        ...
        if (xpathList != null && xpathList.size() > 0) {
            MCRObjectID mcrObjectID = new MCRObjectID(mcrHit.getID());
            Document doc = retrieveXML(mcrObjectID);
            for (int i = 0; i < xpathList.size(); i++) {
                try {
                    ...
                    LOGGER.error("Couldnt parse xpath expression " + xpathList.get(i) + " while creating compare criterias for mcr object "
                            + mcrHit.getID(), jdomExc);
                }
            }
        }
    ...
    private Document retrieveXML(MCRObjectID mcrObjectID) {
        return MCRXMLHelper.parseXML(MCRXMLTableManager.instance().retrieveAsXML(mcrObjectID));
    }
