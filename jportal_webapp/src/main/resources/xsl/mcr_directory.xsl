...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  <xsl:template match="/mcr_directory">
    ...
    <table class="derivateDetails" id="metaData" cellpadding="0" cellspacing="0">
      <tr>
      ...
      <tr>
        ...
        <td class="metavalue">
          <xsl:choose>
            <xsl:when test="numChildren">
              <xsl:value-of select="concat( numChildren/total/files, ' / ', numChildren/total/directories )" />
            </xsl:when>
            <xsl:otherwise>
              <xsl:variable name="numFiles" select="count(children/child[@type='file'])" />
              <xsl:variable name="numDir" select="count(children/child[@type='directory'])" />
              <xsl:value-of select="concat( $numFiles, ' / ', $numDir )" />
            </xsl:otherwise>
          </xsl:choose>
        </td>
      </tr>
    ...
    <table id="files" cellpadding="0" cellspacing="0">
      ...
      <xsl:apply-templates select="." mode="parentDirectory" />
  ...
  <!-- parent directory ********************************************** -->
  <xsl:template match="mcr_directory" mode="parentDirectory">
    <xsl:if test="./parentPath">
      <xsl:variable name="parentdoc" select="document(uri)" />
      <xsl:variable name="derivifs" select="concat($ServletsBaseURL,'MCRFileNodeServlet/',mcr:encodeURIPath(ownerID),$HttpSession)" />
      <tr>
  ...
  <!-- Child componets ********************************************** -->
  <xsl:template match="mcr_directory" mode="children">
    ...
    <xsl:variable name="contentTypes" select="document('resource:FileContentTypes.xml')" />
    <xsl:for-each select="children/child">
      ...
      <xsl:variable name="derivid" select="../../ownerID" />
      ...
      <tr class="{$rowStyle}">
        ...
        <td>
          ...
          <xsl:if test="@type = 'directory'">
            <xsl:variable name="derivifs"
              select="concat($ServletsBaseURL,'MCRFileNodeServlet/',$derivid,mcr:encodeURIPath(concat($path, $derivmain)),$HttpSession)" />
            <a title="{$derivmain}">
          ...
          <xsl:if test="@type = 'file'">
            ...
            <xsl:variable name="toolTipImg"
              select="concat($ServletsBaseURL,'MCRThumbnailServlet/',$derivid,mcr:encodeURIPath(concat($path, $derivmain)),$HttpSession)" />
