...
public class ACLEResource {
    ...
    @GET
    @Path("objectid")
    public void objectid() throws IOException, JDOMException{
        InputStream guiXML = getClass().getResourceAsStream("/jportal_acl_editor_module/gui/xml/objectid.xml");
        SAXBuilder saxBuilder = new SAXBuilder();
        Document webPage = saxBuilder.build(guiXML);
        MCRLayoutService.instance().doLayout(request, response, new MCRJDOMContent(webPage));
    }
