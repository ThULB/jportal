var ACLEditor = function(){
		...
		init: function(ruleSelect, accessTable, ruleList){
			...
			$("body").on("click", ".sort-table-head", function() {
				if($(this).data("sort-dir") == "asc"){
					...
					$(this).children(".sort-icon").addClass("icon-caret-up")
				}
				else{
					$(".icon-caret-up").removeClass("icon-caret-up");
					$(".icon-caret-down").removeClass("icon-caret-down");
					$(this).children(".sort-icon").addClass("icon-caret-down")
				}
			});
	...
	function buildPaginator(page) {
		$(".pagination ul").html("");
		pagecount = Math.floor($("#access-table tbody tr:not(.filter-hide)").size() / $("#elem-per-page-input").val()) + 1;
		if(pagecount > 3){
			if(page > 2){
				if (page < pagecount - 1){
